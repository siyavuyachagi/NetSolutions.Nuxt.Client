            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <div class="form-group">
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-1">
                        Email
                        <span v-if="model.email.errors?.values()" class="mt-1 text-sm text-red-600 ml-2">
                ...
                <div class="form-group">
                    <label for="subject" class="block text-sm font-medium text-gray-700 mb-1">
                        Subject
                        <span v-if="model.subject.errors?.values()" class="mt-1 text-sm text-red-600 ml-2">
                ...
                <div class="form-group">
                    <label for="message" class="block text-sm font-medium text-gray-700 mb-1">
                        Message
                        <span v-if="model.message.errors?.values()" class="mt-1 text-sm text-red-600 ml-2">
...
async function handleSubmit() {
    try {
        ...
        if (state.isValid) {
            await emailService.contactUs(state.formData);
            emit('submitted')
